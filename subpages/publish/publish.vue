			...
			<van-overlay :show="showWarning">
			  <view class="wrapper">
				  <view class="contant">
					...
					<van-button type="primary" size="large" round color="linear-gradient(to right, #ff2950, #f64a06)" @click="hideT">确定</van-button>
				  </view>
			  </view>
			</van-overlay>
		</view>
...
<script setup>
	...
	onLoad((option)=>{
		...
		if('content'in option){
			...
			confirm({detail:from.deliveryTime})
			confirm({detail:from.dateTime})
